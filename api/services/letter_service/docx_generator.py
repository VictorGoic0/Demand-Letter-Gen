...
def html_to_docx(html_content: str) -> Document:
    ...
    try:
        ...
        cleaned_html = re.sub(
            r"^\s*```html\s*", "", cleaned_html, flags=re.IGNORECASE | re.MULTILINE
        )
        ...
        logger.error(f"Failed to convert HTML to DOCX: {e!s}")
        raise ValueError(f"Failed to convert HTML to DOCX: {e!s}") from e


def generate_filename(letter_title: str, date: datetime | None = None) -> str:
    """
    ...
    try:
        ...
        logger.error(f"Failed to generate filename: {e!s}")
        # Fallback filename
...
def save_docx_to_s3(
    ...
    try:
        ...
        logger.error(f"Failed to save DOCX to S3: {e!s}")
