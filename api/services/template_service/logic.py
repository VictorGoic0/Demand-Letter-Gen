...
def get_templates(
    ...
    try:
        ...
        if sort_by == "name":
            order_func = (
                asc(LetterTemplate.name) if sort_order == "asc" else desc(LetterTemplate.name)
            )
        elif sort_by == "created_at" or sort_by is None:
            order_func = (
                asc(LetterTemplate.created_at)
                if sort_order == "asc"
                else desc(LetterTemplate.created_at)
            )
        else:
            # Default to name asc
        ...
        logger.error(f"Error getting templates: {e!s}")
        raise
...
def get_default_template(
    ...
    try:
        ...
        logger.error(f"Error getting default template: {e!s}")
        raise
