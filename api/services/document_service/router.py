...
async def upload_document_endpoint(
    ...
    uploaded_by: UUID | None = Query(
        None, description="User ID who uploaded the document (optional)"
    ),
    ...
    try:
        ...
        logger.error(f"Error in upload endpoint: {e!s}")
        raise HTTPException(
...
async def list_documents_endpoint(
    ...
    sort_by: str | None = Query(None, description="Field to sort by (filename, uploaded_at)"),
...
async def download_document_endpoint(
    ...
    expiration: int = Query(
        3600, ge=60, le=86400, description="URL expiration in seconds (default: 3600, max: 86400)"
    ),
    db: Session = Depends(get_db),
):
    """
    ...
    try:
        ...
        logger.error(f"Error in download endpoint: {e!s}")
        raise
