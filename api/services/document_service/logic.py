...
def get_documents(
    ...
    try:
        ...
        if sort_by == "filename":
            ...
            order_func = (
                asc(Document.uploaded_at) if sort_order == "asc" else desc(Document.uploaded_at)
            )
        else:
...
def get_document_by_id(
    ...
    try:
        ...
        logger.error(f"Error getting document: {e!s}")
        raise
...
def generate_download_url(
    ...
    try:
        ...
        logger.error(f"Error generating download URL: {e!s}")
        raise
