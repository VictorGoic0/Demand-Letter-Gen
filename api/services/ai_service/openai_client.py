...
def call_openai_api(
    ...
    logger.info(
        f"Calling OpenAI API with model={model}, temperature={temperature}, estimated_tokens={estimated_tokens}"
    )
    ...
    for attempt in range(max_retries):
        try:
            ...
            if any(
                ...
                for keyword in ["timeout", "connection", "network", "503", "502"]
            ):
                if attempt < max_retries - 1:
                    logger.warning(
                        f"Transient error, retrying in {delay} seconds (attempt {attempt + 1}/{max_retries}): {error_msg}"
                    )
                    time.sleep(delay)
                    delay *= 2
                    continue
    ...
    if last_exception:
        raise OpenAIException(
            ...
            detail=f"Failed after {max_retries} retries: {last_exception!s}",
        ) from last_exception
...
def validate_response_format(response_text: str) -> bool:
    ...
    return bool("<" in response_text and ">" in response_text)
