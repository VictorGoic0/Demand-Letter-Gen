...
def validate_pdf_structure(file_content: bytes) -> bool:
    ...
    try:
        ...
        logger.error(f"PDF structure validation failed: {e!s}")
        return False
    ...
    try:
        ...
        for page_num, page in enumerate(reader.pages, start=1):
            try:
                ...
                logger.warning(f"Failed to extract text from page {page_num}: {e!s}")
        ...
        logger.error(f"PDF read error: {e!s}")
        raise ValueError(f"Failed to read PDF: {e!s}") from e
    except PdfStreamError as e:
        logger.error(f"PDF stream error: {e!s}")
        raise ValueError(f"Invalid PDF stream: {e!s}") from e
    except Exception as e:
        logger.error(f"Unexpected error extracting text from PDF: {e!s}")
...
def extract_metadata_from_pdf(file_content: bytes) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Error extracting metadata from PDF: {e!s}")
        raise ValueError(f"Failed to extract metadata from PDF: {e!s}") from e
