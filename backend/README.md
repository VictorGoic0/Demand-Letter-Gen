...
Create and use a venv under `backend/.venv` (gitignored):
...
Create **`backend/.env`** for local development (see also `shared/config.py` / `get_settings()`). Example:
...
**Production deploy:** load secrets before Serverless commands, e.g. `source load-env-production.sh` (expects `backend/.env.production`), then `npm run deploy:prod`. See `docs/lambda-deployment.md` and `serverless.yml`.
...
Helpers (from repo root or `backend/`):

- `./migration_scripts/migrate-up.sh`
- `./migration_scripts/migrate-down.sh`
- `./migration_scripts/migrate-create.sh "message"`
...
Or from the backend container: `docker-compose exec backend python test_db.py`.
...
| `scripts/` | One-off checks, seeds, tests |
| `serverless.yml` | Lambda functions and API Gateway |

Architecture rules are documented in **`.cursor/rules/backend-patterns.mdc`** (firm scoping, no direct `os.getenv` in services — use `get_settings()`, S3 via `get_s3_client()`, etc.).
