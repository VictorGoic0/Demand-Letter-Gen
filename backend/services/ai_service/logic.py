...
from .openai_client import build_generation_prompt, call_openai_api, validate_response_format
...
def generate_letter(
    ...
    try:
        ...
        logger.info(
            f"Starting letter generation for firm {firm_id} with template {request.template_id} and {len(request.document_ids)} documents"
        )
        ...
        try:
            ...
            logger.error(f"Error fetching template: {e!s}")
            raise TemplateNotFoundException(
                template_id=str(request.template_id),
                detail=f"Failed to fetch template: {e!s}",
            ) from e
        ...
        for document in documents:
            try:
                ...
                logger.error(f"Error parsing document {document.id}: {e!s}")
                raise ParserException(
                    message="Failed to parse document",
                    detail=f"Error parsing document {document.filename}: {e!s}",
                ) from e
        ...
        try:
            ...
            logger.error(f"Error building prompt: {e!s}")
            raise ValidationException(
        ...
        try:
            ...
            logger.error(f"Unexpected error calling OpenAI API: {e!s}")
        ...
        try:
            ...
            db.rollback()
            logger.error(f"Error creating letter record: {e!s}")
            raise ValidationException(
