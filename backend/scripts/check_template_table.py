...
from shared.models import Firm, LetterTemplate, User
    ...
    try:
        ...
        for template in templates:
            ...
            if template.letterhead_text:
                print(
                    f"Letterhead: {template.letterhead_text[:100]}{'...' if len(template.letterhead_text) > 100 else ''}"
                )
            if template.opening_paragraph:
                print(
                    f"Opening Paragraph: {template.opening_paragraph[:100]}{'...' if len(template.opening_paragraph) > 100 else ''}"
                )
            if template.closing_paragraph:
                print(
                    f"Closing Paragraph: {template.closing_paragraph[:100]}{'...' if len(template.closing_paragraph) > 100 else ''}"
                )
            if template.sections:
                sections_str = (
                    json.dumps(template.sections)
                    if isinstance(template.sections, list)
                    else str(template.sections)
                )
