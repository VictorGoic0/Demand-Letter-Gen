...
from shared.models import Firm, GeneratedLetter, LetterTemplate, User
    ...
    try:
        ...
        for letter in letters:
            ...
            if letter.template_id:
                template = (
                    db.query(LetterTemplate).filter(LetterTemplate.id == letter.template_id).first()
                )
            ...
            if letter.content:
                print(
                    f"Content Preview: {letter.content[:100]}{'...' if len(letter.content) > 100 else ''}"
                )
