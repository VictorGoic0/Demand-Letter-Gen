...
The API is structured for AWS Lambda deployment using the Serverless Framework. Each service endpoint is deployed as a separate Lambda function, sharing common dependencies through Lambda layers.
...
Create a `.env` file in the `api/` directory with all required variables (see `.env.example` for reference).
...
See [`api/handlers/README.md`](../api/handlers/README.md) for detailed handler patterns.
